...
`cjpm build` 配置 `-l, --lint` 选项时，会在编译期间调用仓颉语言静态检查工具 `cjlint` 进行代码检查。如果检查到【要求】级别的代码规范违规，则此次编译会失败，相应检查结果会输出至错误流；检查到【建议】级别的违规时仅会告警，并正常完成编译。`cjlint` 支持检查的代码规则列表及其级别详见 [cjlint 用户手册](./cjlint_manual.md#支持检查的规范列表持续新增中)。
...
`test` 用于编译并运行仓颉文件的单元测试用例，运行后打印测试结果，编译产物默认存放在 `target/release/unittest_bin` 文件夹。单元测试用例代码的写法可参考《仓颉编程语言库 API》中 `std.unittest` 库的说明。
...
`bench` 用于执行测试文件的性能用例并直接打印测试结果。编译产物默认存放在 `target/release/unittest_bin` 文件夹中。性能用例由 `@Bench` 宏标注。更多关于如何编写性能用例代码的详细信息，请参见《仓颉库用户指南》中对 `std.unittest` 库的描述。
...
这里填入的命令会在 `build` 执行时插入到编译命令中间，多个命令可以用空格隔开。可用的命令参考《仓颉编程语言开发指南》的[编译选项](docs/user_manual/source_zh_cn/Appendix/compile_options.md)章节内容。
...
这里填入的命令会在 `build` 执行时插入到编译命令中间，并且拼接于模块配置的 `compile-option` 内容之后，优先级高于 `compile-option`。可用的命令参考《仓颉编程语言开发指南》的[编译选项](docs/user_manual/source_zh_cn/Appendix/compile_options.md)章节内容。
...
>     - 配置该字段的模块，其子包若导入了标准库，并且子包未被 `root` 包依赖，那么 `root` 包中也需要导入对应的标准库，否则会发生无法找到标准库符号的情况。
