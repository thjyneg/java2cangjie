...
## Core Principle: Protected Files (Cannot Be Modified)

**🔴 IRON RULE: Never modify cjpm.toml or remove emptyp.cj files.**

The following files are protected and must never be modified during the fix process:

### Protected Files List

1. **cjpm.toml** - Project configuration file
   - Never modify package name, version, or dependencies
   - Never change compilation options or build settings
   - If cjpm.toml needs changes, this must be done manually by the user

2. **emptyp.cj** - Empty type placeholder files
   - Never remove these files
   - Never modify their content
   - These are generated by j2cj and serve as placeholders

### Why These Files Are Protected

- **cjpm.toml**: Contains project configuration that should only be changed by the user
- **emptyp.cj**: May be referenced by other files; removing them causes compilation errors

### If Modification Is Needed

If you encounter an error that seems to require modifying these files:

1. **Stop** - Do not attempt the modification
2. **Document** - Record the issue in the checkpoint file
3. **Report** - Inform the user that manual intervention is required
4. **Skip** - Move to the next fix and continue
